<x-guest-layout>
    ...
    <div class="bg-l-blue pt-10 pb-5 w-full  -mt-20" style="">
        ...
        <a 
        href="
        @if(Auth::user()->is_penyisihan_done == 0 && Auth::user()->start_penyisihan != null or Auth::user()->is_penyisihan_done == 0)
        {{route('startPenyisihan','1')}}
        @endif
        "
        ><button class="text-center text-4xl font-bold p-5 text-white rounded-md bg-blue-700 hover:bg-blue-800 ">
        @if(Auth::user()->is_penyisihan_done == 1)   
            Sesi Berakhir
        @elseif(Auth::user()->is_penyisihan_done == 0 && Auth::user()->start_penyisihan != null)
            Lanjutkan Sesi Penyisihan
        @elseif(Auth::user()->is_penyisihan_done == 2)
            Babak Penyisihan Belum Dibuka
        @else
